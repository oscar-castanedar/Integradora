...
@section('contenido')
<br>
    <a class="btn btn-primary btn-sm" href="{{route('alumno')}}">Atrás</a>
<br>
<br>
@foreach($cursos as $c)
    ...
    <center><h6>Tu avance:</h6></center>
    <div class="progress">
        <div class="progress-bar" role="progressbar" style="width: {{$u->progreso}}%;" aria-valuenow="{{$u->progreso}}" aria-valuemin="0" aria-valuemax="100">{{$u->progreso}} %</div>
    </div>
    <hr>
<h3>Parcial 1</h3>
<div class="row">
    @foreach($tema as $g)  
      <div class="col-md-3 col-sm-6 m-4">
        <div class="card card-block p-4" >
          ...
          <hr>
          <a href="{{route('entregarActividad',$g->_id)}}" class="btn btn-primary btn-sm">Entregar Actividad</a>
          </form>
        </div>
      </div>
      @endforeach   
